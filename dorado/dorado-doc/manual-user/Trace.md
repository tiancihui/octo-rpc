...
<img src="../../resources/Timeline.svg">
Dorado开源版本提供的监控模块是对Cat（https://github.com/dianping/cat） 的集成，你需要先部署Cat服务才能使用，如果你有自己的性能监控服务，也可以扩展Trace模块（只需要实现InvokeTrace接口）使用自己服务的API做数据上报，InvokeTrace提供了TraceParam参数，开启分阶段埋点你就可以获取框架的耗时统计了。
...
###2.使用说明
>若使用Cat作数据监控，请先部署Cat服务(https://github.com/dianping/cat)
####2.1 基本的埋点上报
...
<bean id="serverPublisher" class="com.meituan.dorado.config.service.spring.ServiceBean">
    <! -- ...... -->
    <property name="registry" value="mock"/>                 <!-- mock方式，用于直连 -->
    <property name="registry" value="mns"/>                  <!-- mns OCTO-NS 命名服务 -->
    <property name="registry" value="zookeeper://ip:port"/>  <!-- zookeeper>
</bean>
````
####2.3 不上报数据
Dorado目前只与Cat集成，若不想做数据埋点，剔除dorado-trace-cat模块
